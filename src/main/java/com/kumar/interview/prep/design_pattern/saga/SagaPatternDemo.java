...
/**
 * Demonstrates orchestration-based saga for a distributed order workflow: CreateOrder -> ProcessPayment ->
 * UpdateInventory -> DeliverOrder.
 *
 * <p>
 * Design highlights:
 * <ul>
 ...
 *
 * <p>
 * See {@code saga-pattern.md} in this package for full documentation.
 */
public class SagaPatternDemo {
    ...
    private static void runSuccessfulOrderSaga() {
        ...
        SagaExecutionResult<OrderSagaContext> result = executePlaceOrderSaga(services,
    ...
    private static SagaExecutionResult<OrderSagaContext> executePlaceOrderSaga(OrderServices services,
        ...
        SagaExecutionCoordinator<OrderSagaContext> coordinator = new SagaExecutionCoordinator<>(
                new LoggingSagaExecutionListener<>());
    ...
    private static void printOutcome(SagaExecutionResult<OrderSagaContext> result, OrderServices services) {
        switch (result) {
            ...
            case SagaExecutionResult.Failure < OrderSagaContext > failure -> {
                ...
                failure.compensationOutcomes().forEach(outcome -> System.out.println("  compensation["
                        + outcome.stepName() + "]=" + (outcome.success() ? "OK" : "FAILED: " + outcome.detail())));
            }
        }
        System.out.println("Remaining inventory for SKU-42: " + services.inventory().availableQuantity("SKU-42"));
    }
}
