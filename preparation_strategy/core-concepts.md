...
## Security and privacy (senior bar)

**OWASP-style themes (know examples):** injection (SQL/command), **broken authentication**, **SSRF**, **mass assignment**, insecure deserialization.

**Secrets:** rotation, vault/KMS—never committed; short-lived credentials where possible.

**Encryption:** at rest (disk/db) + in transit (TLS); **key rotation** and who can decrypt (KMS policies).

**Threat modeling (light):** assets, trust boundaries, entry points; **STRIDE** categories as vocabulary.

**Privacy:** **PII** minimization, retention limits, audit who accessed sensitive data—ties to compliance conversations without needing legal depth.

---

## Databases and transactions (senior depth)

**Isolation recap:** weaker levels allow **dirty reads**, **non-repeatable reads**, **phantom reads**—know which your DB defaults to and **what your feature assumes**.

**MVCC (intuition):** snapshots allow readers without blocking writers; **long transactions** can amplify bloat/visibility anomalies—keep TXNs short.

**Lost updates:** **optimistic** (version column / compare-and-set on commit) vs **pessimistic** (`SELECT … FOR UPDATE`).

**Replicas:** **replication lag** → stale reads unless **read-after-write** routed to leader or verified; **failover** windows and split-brain mitigation.

**CDC:** change-data-capture from WAL/binlog → search index, warehouse, cache invalidation—decouple without polling everything.

---

## Distributed IDs and clocks

**Snowflake / ULID-style:** roughly sortable, unique without central choke point (machine id + time); **clock skew** handling matters.

**Wall clock vs monotonic:** scheduling/timeouts use **monotonic** where available; never assume synchronized wall clocks for **global ordering**.

**Ordering:** **happens-before** and consensus logs beat “timestamp wins” for correctness under concurrency.

---

## Architecture and data evolution

**CQRS:** separate read/write models—helps scale read paths; cost is complexity and eventual consistency between sides.

**Event sourcing:** store facts/events as SSOT; powerful audit/replay; operational complexity (snapshots, schema evolution).

**Outbox** (with Saga): publish reliably after commit; **inbox / idempotent consumer** dedupes duplicate deliveries.

**Bounded context** (DDD-lite): transactional consistency **within** aggregate/service boundary; cross-boundary **Saga**/events.

---

## Webhooks and integrations

Sign (**HMAC**), timestamp **replay window**, **retry** policy, document **idempotency** for receivers; version payload schema.
